...
export default function MovieCard({movie}: any): JSX.Element {
    ...
    return (
        <div className="card bg-dark border-primary me-3" style={{ display: "inline-block", float: "none", width: "15rem", whiteSpace: "normal", verticalAlign: "top", textAlign: "center" }}>
            ...
            <ul className="pe-4 me-2">
                ...
                <hr />
                <p style={{ height: "3rem" }}>Žanr: {movie.Genre}</p>
                <hr />
            ...
            <div className="card-body mt-3">
                <Link to={`/content/movie/${movie._id}`}>
                    <button onClick={putCookies} className="btn btn-primary">Više informacija</button>
                </Link>
            </div>
        </div>
    );
